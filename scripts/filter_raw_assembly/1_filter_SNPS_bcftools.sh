...
bcftools view -S ^samples_removed.txt populations.snps.vcf.gz | bgzip -c > raw_selSpec.snps.vcf.gz
bcftools index raw_selSpec.snps.vcf.gz
#Filter 1:  
...
bcftools +setGT raw_selSpec.snps.vcf.gz -- -t q -n . -i 'FMT/DP <= 3' | \
bcftools +setGT -- -t "b:AD<0.01" -n . | \
...
# Remove SNPs missing in >50% of samples
...
bcftools view -R regions.tmp.txt filter3.tmp.vcf.gz | bgzip -c > ${prefix}_selSpec.vcf.gz
bcftools index ${prefix}_selSpec.vcf.gz
#check how many variants remain
bcftools query -f '%CHROM' ${prefix}_selSpec.vcf.gz | wc -l
#check how many loci remain
bcftools query -f '%CHROM' ${prefix}_selSpec.vcf.gz | sort | uniq | wc -l
